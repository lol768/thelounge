...
<form class="container" method="post" action="" data-event="{{#if defaults.uuid}}network:edit{{else}}network:new{{/if}}">
	...
	{{#if defaults.uuid}}
		...
		<div class="col-sm-9">
			<textarea class="input" id="connect:commands" name="commands" placeholder="One raw command per line, each command will be executed on new connection">
				{{~#each defaults.commands~}}
					{{~this~}}
				{{~/each~}}
			</textarea>
		</div>
